...
export class ApplyCareersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.lang == 'ar') this.survey = new Survey.Model(surveyJSONAr);
    else if (this.lang == 'en') this.survey = new Survey.Model(surveyJSONEn);
    ...
    this._route.params.subscribe((res) => {
      ...
      const date = res?.['date'];
      this.vacancyId = res?.['vacancyId'] || 'null';
      if (email && date && this.vacancyId)
        this.getVacancyProfile(email, date, this.vacancyId);
    });
    this.survey.onCompleting.add((survey, options) => {
      ...
      if (
        this.checkNullableObject(survey.data.UploadedCV) &&
        (survey.data.Documents == null || survey.data.Documents.length <= 0)
      ) {
      ...
      if (
        this.checkNullableObject(survey.data.UploadedCV) &&
        !this.checkNullableObject(survey.data.OtherDocuments)
      ) {
        options.allowComplete = false;
        this.uploadCvErrorMsg = true;
      }
      ...
      if (
        !this.checkNullableObject(survey.data.OtherDocuments) &&
        !this.CheckValidityFilesExtensions(survey.data.OtherDocuments)
      ) {
        options.allowComplete = false;
      ...
      if (
        !this.checkNullableObject(survey.data.UploadedCV) &&
        !this.CheckValidityFilesExtensions(survey.data.UploadedCV)
      ) {
        options.allowComplete = false;
        this.extensionErrorMsg = true;
      }
      ...
      if (
        !this.checkNullableObject(survey.data.OtherDocuments) &&
        survey.data.OtherDocuments.length > 2
      ) {
        options.allowComplete = false;
    ...
    this.survey.onComplete.add((survey, options) => {
      ...
      this._careersService.AddVacancy(this.body).subscribe(
        (data) => {
          this.success = true;
          this._router.navigate['/careers-Opportunities/careers-list'];
        },
        (error) => {
  ...
  getVacancyProfile(email, date, vacancyId) {
    this._careersService.GetVacancyProfile(email, date, vacancyId).subscribe(
      (data) => {
        if (data) {
          this.data = data;
          this.careers = this.data.CareersViewModel;
          ...
          if (!this.checkNullableObject(data.JoinedUsAsLst)) {
            ...
            if (this.lang == 'ar') {
              if (data.JoinedUsAsLst == 'Job') data.JoinedUsAsLst = 'وظيفة';
              ...
              if (data.JoinedUsAsLst == 'وظيفة') data.JoinedUsAsLst = 'Job';
          ...
          if (
            this.survey.getValue('LinkToPortfolio') == 'undefined' ||
            this.survey.getValue('LinkToPortfolio') == 'null'
          )
  ...
  afterRenderPanel = (survey, options) => {
    ...
    // if (options.panel.question.hideNumber) {
    //   var checkDiv = options.htmlElement.querySelector(".sv-question");
    //   checkDiv.className = "customQuestion";
    // }
  };
  afterRenderQuestion = (survey, options) => {
    if (options.question.icon) {
      ...
      var btnLabel = options.htmlElement.querySelector(
        '.sv-file__choose-btn span'
      );
      ...
      var descCont = options.htmlElement.querySelector(
    ...
    if (options.question.hideNumber) {
      ...
      if (checkTitle != null) checkTitle.className = 'hideTitle';
      var checkDiv = options.htmlElement.querySelector('.sv-question');
      if (checkDiv != null) checkDiv.className = 'customQuestion';
    }
    if (options.question.inputType == 'date') {
      var checkDate = options.htmlElement.querySelector(
  ...
  checkNullableObject(arr: any[]) {
    if (arr != undefined) {
      if (arr) {
        if (arr.length > 0) {
          if (Object.keys(arr[0]).length === 0) return true;
          else return false;
        }
      }
    }
  ...
  GenerateFormData(data) {
    ...
    this.formData.append('VacancyId', this.vacancyId);
    this.formData.append('RefrenceNum', this.careers.ReferenceNumber);
    ...
    if (!this.checkNullableObject(data.AcademicDegrees))
      this.formData.append(
        'AcademicDegreesStr',
        JSON.stringify(data.AcademicDegrees)
      );
    ...
    if (!this.checkNullableObject(data.UploadedCV))
      this._helper.appendArrayToFormData(
        this.formData,
        'UploadedCV',
        data.UploadedCV
      );
  ...
  CheckValidityFilesExtensions(arr: any) {
    for (var item of arr) {
      ...
      if (
        extensions[extensions.length - 1] != 'pdf' &&
        extensions[extensions.length - 1] != 'doc' &&
  ...
  showResponse(response) {
    ...
    this._careersService
      .ValidateRecaptcha({ UserResponse: response?.response })
      .subscribe((data) => {
