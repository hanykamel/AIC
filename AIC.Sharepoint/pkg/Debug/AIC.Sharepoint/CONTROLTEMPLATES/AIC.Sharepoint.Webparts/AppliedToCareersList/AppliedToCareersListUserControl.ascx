...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="AppliedToCareersListUserControl.ascx.cs" Inherits="AIC.Sharepoint.Webparts.AppliedToCareersList.AppliedToCareersListUserControl" %>
...
<script type="text/javascript" src="/_catalogs/masterpage/AIC/JS/jquery.table2excel.min.js"></script>
...
<div class="filtering">
    <form>
        ...
        <select class="textbox" name="careers" id="careers" >
...
<script>	
    ...
    $(document).ready(function () {
        ...
        $.ajax({
            url: _spPageContextInfo.webAbsoluteUrl + '/_vti_bin/AICService.svc/GetCareersList',
            method: 'GET',
            contentType: "application/json; charset=utf-8",
            async: false,
            headers: { "accept": "application/json;odata=verbose", "lang": "en" },
            success: function (data) {
                data = JSON.parse(data.GetCareersListResult);
                console.log(data);
            },
            error: function () {
            }
        ...
        $('#AppliedTableContainer').jtable({
            ...
            actions: {
                listAction: function (postData, jtParams) {
                    if (!postData) {
                        ...
                        postData = "?PageSize=" + jtParams.jtPageSize + "&PageIndex=" + jtParams.jtStartIndex / jtParams.jtPageSize
                            + "&Email=" + $('#email').val() + "&University=" + $('#Universty').val() + "&Age=" + $('#Age').val() + "&AppliedDateFrom=" + $('#dateFrom').val()
                            + "&AppliedDateTo=" + $('#dateTo').val() + "&VacancyId=" + $('#careers').val() + "&RefNum=" + $('#referenceNumber').val();
                    }
                    return $.Deferred(function ($dfd) {
                        
                        $.ajax({
                            url: _spPageContextInfo.webAbsoluteUrl + '/_vti_bin/AICService.svc/GetAppliedForCareersList' + postData,
                            method: 'GET',
                            contentType: "application/json; charset=utf-8",
                            async: false,
                            ...
                            success: function (data) {
                                CVsURLs = [];
                                data = JSON.parse(data.GetAppliedForCareersListResult);
            ...
            fields: {
                ...
                VacancyTitleEn: {
                    ...
                    sorting: true,
                    display: function (record) {
                        return record.record.VacancyTitleEn;
                    }
                },
                ViewButton: {
                    title: 'View Details',
                    list: true,
                    display: function (record) {
                        if (record.record.CVURL != null)
                            CVsURLs.push(record.record.CVURL);
                        return '<a href="/Pages/AppliedToCareerUserDetails.aspx?UserId=' + record.record.Id + '">View</a>';
                    }
                },
                DeleteButton: {
                    title: 'Delete',
    ...
    function DeleteUser(id) {
        ...
        if (isExecuted) {
            return $.Deferred(function ($dfd) {
                $.ajax({
                    url: _spPageContextInfo.webAbsoluteUrl + '/_vti_bin/AICService.svc/DeleteAppliedForCareerById?Id=' + id,
                    method: 'GET',
                    contentType: "application/json; charset=utf-8",
                    async: false,
                    headers: { "accept": "application/json;odata=verbose", "lang": "en" },
                    success: function (data) {
                        data = JSON.parse(data.DeleteAppliedForCareerByIdResult);
    ...
    function compressed_img(urls, nombre) {
        ...
        urls.forEach(function (url) {
            JSZipUtils.getBinaryContent(url, function (err, data) {
                ...
                s = url.split('/');
                path = s[s.length - 2] + "/" + s[s.length - 1];
