...
<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="JoinUsUserDetailsUserControl.ascx.cs" Inherits="AIC.Sharepoint.Webparts.JoinUsUserDetails.JoinUsUserDetailsUserControl" %>
...
<div id="contnet">
    ...
    <div id="AttachmentsContainer" class="contain">
        ...
        <p id="attachTitle" class="info">Attachments</p>
...
<script>
    ...
    function GetUserDetails(id) {
        return $.Deferred(function ($dfd) {
            $.ajax({
                url: _spPageContextInfo.webAbsoluteUrl + '/_vti_bin/AICService.svc/GetAppliedJoinUsById?Id=' + id,
                method: 'GET',
                contentType: "application/json; charset=utf-8",
                async: false,
                headers: { "accept": "application/json;odata=verbose", "lang": "en" },
                success: function (data) {
                    data = JSON.parse(data.GetAppliedJoinUsByIdResult);
                    ...
                    if (result.AcademicDegrees.length > 0) {
                    ...
                    result.Certificates.forEach(Certificate => {
                        $("#Certificates").append(`<tr>
                                                       ...
                                                       <td> ${new Date(Certificate.CertifiedFrom).toLocaleDateString()}</td>
                                                       </tr>`)
                    });
                    ...
                    if ((result.LinkToPortfolio == null || result.LinkToPortfolio == "" || result.LinkToPortfolio == "null" || result.LinkToPortfolio == "undefined") &&
                        (result.JoinedUsAs == null || result.JoinedUsAs == "" || result.JoinedUsAs == "") &&
                        (result.JoinedIn == null || result.JoinedIn == "" || result.JoinedIn == "")) {
                        $("#OtherInfo").hide();
                    }
                    else {
                    ...
                    if (result.LinkToPortfolio == null && result.JoinedUsAs == null && result.JoinedIn == null && result.Documents.length == 0) {
